...
export function setupLoveSpell(scene, {spellIngredients, cauldron, centerX, centerY, tooltipIngredient}) {
 ...
 handleOkPotionBtn(scene)
  ...
  const spellWork= [
    ...
    {
      jar:'pink-salt-jar',
      ingredient: 'pink-salt-ingredient',
      x:centerX - 120,
      y: centerY - 190,
      scale: 0.2,
      name: 'Pink Salt'
    ...
    {
      jar:'rose-petals-jar',
      ingredient:'rose-petal-ingredient',
      x: centerX - 320,
      y: centerY - 180,
      scale: 0.2,
      name: 'Rose Petals'
      ...
      scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if (withinCauldron) {
          ...
          switch(gameObject.texture.key) {
            ...
            break;
            case 'pink-salt-ingredient': stepCompletion[1]= true;
            break;
            case 'rose-petal-ingredient': stepCompletion[2]= true;
            break;
  ...
  const cauldronOpenCirclee = new Phaser.Geom.Circle(
     ...
     scene.input.on('pointermove', (pointer, gameObject) => {
      ...
      if(isStirring) {
        ...
        if(moonWaterIngredient1) {
          ...
          moonWaterIngredient1.rotation += 0.01
        }

        if (Phaser.Math.Between(0, 100) < 50) {
          const bubbleGroup= scene.add.group()
          const colorBubble= 'red-bubble'
          const tint= '0xff0000'
   ...
   if (!scene.anims.exists('cauldron-water-love')) {
      scene.anims.create({
        key: 'cauldron-water-love',
        frames: scene.anims.generateFrameNumbers('stirring-animation-love', {start: 0, end:3}),
        frameRate: 0.5,
        repeat: 0
       })
      }
     ...
     const stirringLiquid= scene.add.sprite(cauldronOpenCircle.x - 5, cauldronOpenCircle.y - 27, 'stirring-animation-love').setScale(0.3).setDepth(5).setVisible(false)
    ...
    const brewButton= document.querySelector('.brew-btn')
      ...
      brewButton.addEventListener('click', () => {
        ...
        stirringLiquid.play('cauldron-water-love')
      ...
      const redWater= scene.add.image(centerX + 183, centerY + 95, 'red-water').setScale(0.3).setInteractive().setOrigin(0.5, 0.5).setVisible(false)
       ...
       cauldron.on('pointerdown', () => {
        ...
        redDrop= scene.add.image(centerX - 300, centerY + 100, 'red-drop').setScale(0.2).setInteractive()
        stirringLiquid.setVisible(false)

        redDrop.on('pointerover', () => {
         scene.input.setDefaultCursor('grab')
        })
  
        redDrop.on('pointerout', () => {
         scene.input.setDefaultCursor('grab')
        })

        scene.input.setDraggable(redDrop)
       ...
       scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if(withinEmptyJar && gameObject === redDrop ){
        
           scene.input.setDefaultCursor('grab')
    

         redWater.setVisible(true).setScale(0.15)
           gameObject.destroy()

           stepCompletion[7]= true
           console.log('red potion jar added')
       ...
       scene.add.dom(centerX + 480, centerY - 90).createFromHTML(`
        <div class="love-heading">Instructions</div>
      `)

        const instructionPaper=  scene.add.dom(centerX + 480, centerY + 270).createFromHTML(`
          <div class="btn-cont">
          <button class="complete-btn-love complete-btn-all">Complete</button>
          </div>
       ...
       const textElements= [
        ...
        'Add pink salt into the cauldron',
        'Add rose petals into the cauldron',
       ...
       let currentY = centerY - 70;
        ...
        scene.time.delayedCall(0, () => {
          ...
          phaserTextObjects.forEach((textObj, index) => {
            ...
            const whiteHeart = scene.add.image(centerX + 295, yPosition, 'white-heart')
              .setScale(0.05)
            ...
            whiteHeart.on('pointerdown', () => {
              if (stepCompletion[whiteHeart.stepIndex]) {
                whiteHeart.setTexture('red-heart').setScale(0.03);
       ...
       instructionPaper.addListener('click')
        ...
        if(event.target.classList.contains('complete-btn-love')) {
        
          const stepsCompleted= stepCompletion.every(val => val === true) 
          const allRedHearts= whiteHearts.every(heart => heart.texture.key === 'red-heart')
          ...
          if(!allRedHearts) {
           handleHeartsMsg(scene)
            ...
            scene.scene.start('FinalScene', { spell: 'Love' });
          }
        }
