...
export function setupProtectionSpell(scene, {spellIngredients, cauldron, centerX, centerY, tooltipIngredient}) {
 ...
 handleOkPotionBtn(scene)
  ...
  const spellWork= [
    ...
    {
      jar:'rosemary-jar',
      ingredient:'rosemary-ingredient',
      x: centerX - 320,
      y: centerY - 190,
      scale: 0.22,
      name: 'Rosemary'
      ...
      scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if (withinCauldron) {
          ...
          switch(gameObject.texture.key) {
            case 'salt-ingredient': stepCompletion[0]= true;
            break;
            case 'rosemary-ingredient': stepCompletion[1]= true;
            break;
            case 'cloves-ingredient': stepCompletion[2]= true;
            break;
  ...
  const cauldronOpenCirclee = new Phaser.Geom.Circle(
    ...
    openRadiuss
     ...
     scene.input.on('pointermove', (pointer, gameObject) => {
      ...
      if(isStirring) {
       scene.input.setDefaultCursor('grabbing');
        ...
        if(!scene.anims.exists('cauldron-water-protection')) { 
      scene.anims.create({
        key: 'cauldron-water-protection',
        frames: scene.anims.generateFrameNumbers('stirring-animation-protection', {start: 0, end:3}),
        frameRate: 0.5,
        repeat: 0
       })
      }
     ...
     const stirringLiquid= scene.add.sprite(cauldronOpenCircle.x - 5, cauldronOpenCircle.y - 27, 'stirring-animation-protection').setScale(0.3).setDepth(5).setVisible(false)
    ...
    const brewButton= document.querySelector('.brew-btn')
      ...
      brewButton.addEventListener('click', () => {
        ...
        stirringLiquid.play('cauldron-water-protection')
      ...
      const pinkWater= scene.add.image(centerX + 180, centerY + 95, 'pink-water').setScale(0.3).setInteractive().setOrigin(0.5, 0.5).setVisible(false)
       ...
       cauldron.on('pointerdown', () => {
        ...
        if(!whiteDrop) {
          whiteDrop= scene.add.image(centerX - 300, centerY + 100, 'white-drop').setScale(0.3).setInteractive({cursor: 'grab'})

          stirringLiquid.setVisible(false)

          whiteDrop.on('pointerover', () => {
            scene.input.setDefaultCursor('grab');
          })
    
          whiteDrop.on('pointerout', () => {
            scene.input.setDefaultCursor('grab');
          })
  
          scene.input.setDraggable(whiteDrop)
  
          scene.pickIngredient.play()
        } else{
          console.log('cannot get twice')
        }
       ...
       scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if(withinEmptyJar && gameObject === whiteDrop ){
        
          scene.input.setDefaultCursor('grab');

           pinkWater.setVisible(true).setScale(0.2)
           gameObject.destroy()

           stepCompletion[7]= true
           console.log('white potion jar added')
       ...
       scene.add.dom(centerX + 480, centerY - 90).createFromHTML(`
        <div class="protection-heading">Instructions</div>
      `)

      const instructionPaper=  scene.add.dom(centerX + 480, centerY + 270).createFromHTML(`
        <div class="btn-cont">
        <button class="complete-btn-protection complete-btn-all">Complete</button>
        </div>
        ...
        const textElements= [
          'Add salt into the cauldron',
          'Add rosemary into the cauldron',
          'Add cloves into the cauldron',
            ...
            whiteHeart.stepIndex= index
             ...
             whiteHeart.on('pointerdown', () => {
               ...
               if(stepCompletion[whiteHeart.stepIndex]) {
                 whiteHeart.setTexture('pink-heart').setScale(0.05)
       ...
       instructionPaper.on('click', (event) => {
        if(event.target.classList.contains('complete-btn-protection')) {
        
          const stepsCompleted= stepCompletion.every(val => val === true) 
          const allRedHearts= whiteHearts.every(heart => heart.texture.key === 'pink-heart')
          ...
          if(!allRedHearts) {
           handleHeartsMsg(scene)
            ...
            scene.scene.start('FinalScene', { spell: 'Protection' });
          }
        }
