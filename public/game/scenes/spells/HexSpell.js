...
export function setupHexSpell(scene, {spellIngredients, cauldron, centerX, centerY, tooltipIngredient}) {
 ...
 handleOkPotionBtn(scene)
  ...
  let scroll;
  let mudWaterIngredient1;


  const spellWork= [
      
    {
      jar:'rotten-lemons-jar',
      ingredient:'rotten-lemon-ingredient',
      x: centerX, 
      y: centerY + 180, 
      scale: 0.15,
      name: 'Rotten Lemons'
    ...
    {
      jar:'chilli-flakes-jar',
      ingredient:'chilli-flakes-ingredient',
      x: centerX - 300,
      y: centerY - 180,
      scale: 0.18,
      name: 'Chilli Flakes'
    ...
    {
      jar:'rusted-nails-jar',
      ingredient:'rusted-nail-ingredient',
      x: centerX + 10,
      y: centerY + 25,
      scale: 0.15,
     name: 'Rusted Nails'
    ...
    {
      jar:'mud-water-jar',
      ingredient:'mud-drop-ingredient',
      x:centerX - 130,
      y: centerY - 190,
      scale: 0.25,
      name: 'Mud Water'
  ...
  const spellJar= scene.add.image(spell.x, spell.y, spell.jar).setScale(spell.scale). setInteractive({ cursor: 'pointer' }).setOrigin(0.5, 0.5)
  ...
  spellJar.on('pointerdown', () => {
    ...
    if(!ingredientExists) {
      const ingredient= scene.add.image(spell.x, spell.y, spell.ingredient)
      .setScale(0.1)
      ...
      scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if (withinCauldron) {
          ...
          switch(gameObject.texture.key) {
            case 'rotten-lemon-ingredient': stepCompletion[0]= true;
            break;
            case 'chilli-flakes-ingredient': stepCompletion[1]= true;
            break;
            case 'rusted-nail-ingredient': stepCompletion[2]= true;
            break;
            case 'mud-drop-ingredient': 
         
              gameObject.setVisible(false)

              if(!mudWaterIngredient1) {
                mudWaterIngredient1= scene.add.image(
                  cauldronOpenCircle.x - 5,
                  cauldronOpenCircle.y - 30,
                  'mud-water-ingredient1'
                ).setScale(0.46).setDepth(5).setAlpha(0.3)
            ...
            if(gameObject.texture.key === 'mud-drop-ingredient') {
    ...
    const manifestInput= scene.add.dom(centerX + 130, centerY - 170).createFromHTML(`
     ...
     manifestInput.on('click', (event) => {
      if(event.target.className === 'fold') {
        ...
        if(!scroll) {
          scroll= scene.add.image(centerX + 130, centerY - 170, 'scroll').setScale(0.35).setInteractive({cursor: 'pointer'}).setOrigin(0.5, 0.5).setDepth(2)
  ...
  const cauldronOpenCirclee = new Phaser.Geom.Circle(
    ...
    openRadiuss
     ...
     const spoon= scene.add.image(centerX - 530, centerY - 150  , 'spoon').setScale(0.25).setInteractive().setOrigin(0.5, 0.5).setDepth(1)
     ...
     scene.input.on('pointermove', (pointer, gameObject) => {
      ...
      if(isStirring) {
        ...
        if(mudWaterIngredient1) {
          mudWaterIngredient1.setOrigin(0.5, 0.5).setScale(0.45)
          mudWaterIngredient1.rotation += 0.05
        }

        if (Phaser.Math.Between(0, 100) < 50) {
          const bubbleGroup= scene.add.group()
          const colorBubble= 'black-bubble'
          const tint= '0x4b0082'
       ...
       if(!scene.anims.exists('cauldron-water-hex')) {
      scene.anims.create({
        key: 'cauldron-water-hex',
        frames: scene.anims.generateFrameNumbers('stirring-animation-hex', {start: 0, end:3}),
        frameRate: 0.5,
        repeat: 0
       })
      }
     ...
     const stirringLiquid= scene.add.sprite(cauldronOpenCircle.x - 5, cauldronOpenCircle.y - 27, 'stirring-animation-hex').setScale(0.3).setDepth(5).setVisible(false)
    ...
    const brewBtn= scene.add.dom(10, 450).createFromHTML(`
     ...
     const brewButton= document.querySelector('.brew-btn')
      ...
      brewButton.addEventListener('click', () => {
        ...
        stirringLiquid.play('cauldron-water-hex')
       ...
       spellIngredients.forEach(ingredient => ingredient.setVisible(false))
        ...
        if(mudWaterIngredient1) {
          mudWaterIngredient1.setVisible(false)
      ...
      const blackWater= scene.add.image(centerX + 180, centerY + 95, 'black-water').setScale(0.3).setInteractive().setOrigin(0.5, 0.5).setVisible(false)
       ...
       cauldron.on('pointerdown', () => {
        ...
        if(!blackDrop) {
         ...
         blackDrop= scene.add.image(centerX - 300, centerY + 100, 'black-drop').setScale(0.2).setInteractive()
        stirringLiquid.setVisible(false)

        scene.input.setDraggable(blackDrop)

        blackDrop.on('pointerover', () => {
            scene.input.setDefaultCursor('grab')
        })
  
        blackDrop.on('pointerout', () => {
            scene.input.setDefaultCursor('grab')
        })

        scene.pickIngredient.play()
      } else {
        console.log('cannot add twice')
      }
       ...
       scene.input.on('dragend', (pointer, gameObject) => {
        ...
        if(withinEmptyJar && gameObject === blackDrop){
           scene.input.setDefaultCursor('grab')
       
        blackWater.setVisible(true).setScale(0.15)
           gameObject.destroy()

           stepCompletion[7]= true
           console.log('black potion jar added')
       ...
       scene.add.dom(centerX + 480, centerY - 90).createFromHTML(`
        <div class="hex-heading">Instructions</div>
      `)

      const instructionPaper=  scene.add.dom(centerX + 480, centerY + 270).createFromHTML(`
        <div class="btn-cont">
        <button class="complete-btn-hex complete-btn-all">Complete</button>
        </div>
       ...
       const textElements= [
        'Add a rotten lemon into the cauldron',
        'Add chilli flakes into the cauldron',
        'Add a rusted nail into the cauldron',
        'Write a wish and fold the paper and place it in the cauldron',
        'Add mud water into the cauldron',
        'Mix the ingredients',
        'Start brewing',
            ...
            whiteHeart.stepIndex= index
             ...
             whiteHeart.on('pointerdown', () => {
               ...
               if(stepCompletion[whiteHeart.stepIndex]) {
                 whiteHeart.setTexture('black-heart').setScale(0.04)
       ...
       instructionPaper.on('click', (event) => {
        if(event.target.classList.contains('complete-btn-hex')) {
        
          const stepsCompleted= stepCompletion.every(val => val === true) 
          const allRedHearts= whiteHearts.every(heart => heart.texture.key === 'black-heart')
          ...
          if(!allRedHearts) {
           handleHeartsMsg(scene)
            ...
            scene.scene.start('FinalScene', { spell: 'Hex' });
          }
        }
